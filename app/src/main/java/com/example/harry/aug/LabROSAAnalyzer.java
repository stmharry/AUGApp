...
public class LabROSAAnalyzer extends Analyzer {
    ...
    public float mode(float[] in) {
        ...
        float modeLast = Util.sum(in) / length, mode = modeLast;
    ...
    private void analyzeTempo() {
        ...
        envFFT = new FFT(envFFTFrameSize);

        Util.whiten(onsetEnvelope);
        float[] bpmSample = new float[selectionSize];
        for(selection = 0; selection < selectionSize; selection++) {
            float[] onsetSegment = select(onsetEnvelope, envFrameSize);
            float[] onsetCorr = Arrays.copyOf(xcorr(onsetSegment), envFrameSize);
            window(onsetCorr);

            bpmSample[selection] = 60 * envSampleRate / Util.maxIndex(onsetCorr, 0);
        }

        bpm = mode(bpmSample);
        beatScore = score(bpmSample, bpm);

        Log.d(TAG, String.format("[BPM = %.1f][BeatScore = %.3f]", bpm, beatScore));
    }
    ...
    private void analyzeBeat() {
        ...
        for(int i = 0; i < frame; i++) {
            ...
            float[] max = Util.max(scoreThis, 0);
        ...
        beat[beatIndex] = Util.maxIndex(score, 0);
        do {
            beat[beatIndex + 1] = parent[beat[beatIndex]];
        } while(beat[++beatIndex] >= 0);

        beat = flip(Arrays.copyOf(beat, beatIndex));
        beatTime = new long[beatIndex];
        for(int i = 0; i < beatIndex; i++) {
            beatTime[i] = (long)((beat[i] / envSampleRate) * S_TO_US + fftFrameSizeUs * window.maxSlope());
        }
    }
